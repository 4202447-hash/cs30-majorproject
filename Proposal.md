# Major Project Proposal - Metroidvania
I am going to attempt to make a 2d platformer game with combat elements, attempting to replicate games such as Hollow Knight, or Blasphemous 2. The character will be able to explore the map fighting multiple types of enemies, obtaining different items to utilize for combat and movement. I would like to have a story of some sort even if it is minor and small. The game should be a challenging test of reaction speed and pattern recognition. It should have strong artistic appeal.

## Need to have
- **A realistic physics engine** allowing for movement of player, enemies, and projectiles //Done
- **A uniqe health system** which rewards the player for certain combat mechanics //Done (maybe add some effects tho)
- **Atleast 3** unique enemies, one of which should be flying, and another which should release projectiles. //Done 
- **A consistent art scheme** //Yeah theres a theme there
- **Atleast one boss fight**
- **Non-linear map building** allowing the player to go in multiple directions //WIP
- **An ingame map** //WIP
- **Checkpoint & Saving system** //Done current stage saves
- **Fast paced movement** //Done with sprinting and rolling and everything
- **8 - 12 Unique stages / levels** //2 done
- **Atleast one weapon** option (not fists) which provides a uniqe gameplay experience compared to bae fists//havnet started
- **Sound** //havent started

##Nice to have
- Special abilities/attacks
- A higher difficult enemy/mini boss
- A second area with a different theme
- A second boss
- A cutscene (probably would be AI generated)
- A uniqe story
- A system which allows the player to customize their playing experience with selected buffs/alterations of abilities/stats
- Lighting effects (think dark rooms or caves) //Done
- **Items** for the player to hold in their **inventory**
- **A shop** for the player to buy items